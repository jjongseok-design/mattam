...
아래 JSON 형식으로만 답변해 (마크다운 코드블록 없이 순수 JSON만):
{
  ...
  "review_count": 리뷰수(정수)
   ...
   case "fetch_naver_images": {
          ...
          for (const restaurant of targets) {
            try {
              const query = encodeURIComponent(`${restaurant.name} 춘천 맛집`);
              const searchRes = await fetch(`https://openapi.naver.com/v1/search/blog.json?query=${query}&display=5`, {
                headers: { "X-Naver-Client-Id": naverClientId, "X-Naver-Client-Secret": naverClientSecret },
              });
              if (!searchRes.ok) { results.push({ id: restaurant.id, name: restaurant.name, success: false, error:
  "네이버 검색 API 오류" }); continue; }
              const items = (await searchRes.json()).items ?? [];
              if (items.length === 0) { results.push({ id: restaurant.id, name: restaurant.name, success: false, error:
  "블로그 결과 없음" }); continue; }

              let imageUrl: string | null = null;
              for (const item of items) {
                try {
                  const blogRes = await fetch(item.link, { headers: { "User-Agent": "Mozilla/5.0" }, redirect: "follow"
  });
                  if (!blogRes.ok) continue;
                  const html = await blogRes.text();
                  const ogMatch = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i)
                    || html.match(/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']og:image["']/i);
                  if (ogMatch?.[1]) { imageUrl = ogMatch[1]; break; }
                } catch { continue; }
              }

              if (!imageUrl) { results.push({ id: restaurant.id, name: restaurant.name, success: false, error: "이미지를
   찾을 수 없음" }); continue; }

              const imgRes = await fetch(imageUrl, { headers: { "User-Agent": "Mozilla/5.0" } });
              if (!imgRes.ok) { results.push({ id: restaurant.id, name: restaurant.name, success: false, error: "이미지
  다운로드 실패" }); continue; }
  ...
  supabase.storage.from("restaurant-images").upload(`${restaurant.id}.${ext}`, imageBuffer, { contentType, upsert: true
  });
              if (uploadErr) { results.push({ id: restaurant.id, name: restaurant.name, success: false, error:
  uploadErr.message }); continue; }

              const { data: urlData } =
  supabase.storage.from("restaurant-images").getPublicUrl(`${restaurant.id}.${ext}`);
